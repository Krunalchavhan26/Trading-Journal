...
const AllOrderbooks = () => {
  ...
  const handleDeleteClick = (e, orderId, orderPair) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.delete(
        `api/v1/orderbooks/orderbook/${deleteDialog.orderId}`
      );
      ...
      if (response?.data.success) {
        ...
        const updatedOrderbooks = orderbooks.filter(
          (order) => order._id !== deleteDialog.orderId
        );
        dispatch(setOrderbooks(updatedOrderbooks));

        // ✅ Close dialog
        setDeleteDialog({ isOpen: false, orderId: null, orderPair: null });
      }
    } catch (error) {
      console.error("Failed to delete orderbook: ", error);
      toast.error("Failed to delete orderbook");
    } finally {
      setIsDeleting(false);
    }
  };
          ...
          <div className="text-center py-16">
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-8 max-w-md mx-auto">
              ...
              <p className="text-slate-400 text-lg">
                No orderbook entries found.
              </p>
              <p className="text-slate-500 text-sm mt-2">
                Start by creating your first orderbook entry.
              </p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {orderbooks.map((order) => (
              ...
              >
                {/* Delete Button */}
                <button
                  onClick={(e) => handleDeleteClick(e, order._id, order.pair)}
                  className="absolute top-2 left-2 z-10 bg-red-500/20 hover:bg-red-500/30 text-red-400 hover:text-red-300 p-2 rounded-full transition-all duration-200 opacity-0 group-hover:opacity-100 hover:scale-110"
                  title="Delete orderbook"
                >
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                      ...
                      <span>
                        Open: {new Date(order.open).toLocaleDateString()}
                      </span>
                      <span className="text-slate-500">
                        {new Date(order.open).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {/* Session */}
                    <span
                      className={`px-2 py-1 rounded-md text-xs font-medium flex items-center gap-1 border ${getSessionColor(
                        order.session
                      )}`}
                    >
                  ...
                  <div className="flex gap-2">
                    ...
                    <span
                      className={`px-2 py-1 rounded-md text-xs font-medium flex items-center gap-1 border ${
                  ...
                  {order.tradeStatus && (
                    <div className="flex gap-2">
                      <span
                        className={`px-2 py-1 rounded-md text-xs font-medium ${getCloseReasonColor(
                          order.tradeStatus
                        )}`}
                      >
                        {order.tradeStatus}
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
              ...
              <div className="mb-6">
                <p className="text-slate-300 mb-2">
                  Are you sure you want to delete this orderbook?
                </p>
                <div className="bg-slate-700/50 rounded-lg p-3 border border-slate-600">
                  <p className="text-emerald-400 font-medium">
                    {deleteDialog.orderPair}
                  </p>
